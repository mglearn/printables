#!/usr/bin/env bash
# Generates a print-ready 2-page PDF next to every station-lab PNG (same name, .pdf):
#   page 1 = the original color station sheet
...
# (needed after changing the output format). Run from tgt/phase_change_stations/.
set -euo pipefail
cd "$(dirname "$0")"
